...
unsigned int 	texture[32];	// for the cursor icon (newer)
...
int main(int argc, char *argv[])
{
  ...
  // after compiling the latest version of GLUT from
  // http://developer.apple.com/samplecode/Sample_Code/Graphics_3D/glut.htm
  // and then compiling it and having to copy the new Framework by using the
  // 'ditto' command, things worked okay after that!
...
GLvoid drawShapes(GLenum mode)
{
    ...
    if (GL_SELECT == mode)
    {
        ...
        // http://mecadserv1.technion.ac.il/public_html/LabCourses/interActiveGraphics/ogl_course/ogl_lab/ogl_lighting.htm
        // look here for some stuff on lighting...perhaps for the sphere.
        ...
        // I'm trying to get the sphere to change colors randomly,
...
void InitGL()
{
	...
	LoadGLTextures(texture);
        ...
        // http://www.cs.man.ac.uk/applhax/OpenGL/glut.html
        //glutWarpPointer(800, 10);
        glutWarpPointer(kWindowWidth/2, kWindowHeight/2); // move the cursor to the set (x,y) coordinates
...
void specialKeys(int key, int x, int y)
{
	...
	case GLUT_KEY_UP :
                z_camera += 0.05; 
                // x_camera = tan (double(y_angle)) * z_camera;
                glutPostRedisplay();
		break;
	case GLUT_KEY_DOWN :
                z_camera -= 0.05;
...
// ===================================================================
// void directionalKeys(int key)
// This provides functionality so the user can press two different
// directional keys at once and try and move in a resultant vector
// or direction of those two directions
// =================================================================== 
void directionalKeys(int key)
{
    // a directional key is already being pressed down
    if (isDirectionalKeyDown == true)
    {
        switch (key)
        {
            case GLUT_KEY_RIGHT:
                if (directional_key == GLUT_KEY_UP)
                {
                    z_camera += 0.05;
                }
                else if (directional_key == GLUT_KEY_UP)
                {
                    z_camera -= 0.05;
                }
                y_angle = (y_angle + 5) % 360;
		...
		glutPostRedisplay();
                break;                
            case GLUT_KEY_UP :
                if (directional_key == GLUT_KEY_RIGHT)
                {
                    y_angle = (y_angle + 5) % 360;

                }
                else if (directional_key == GLUT_KEY_LEFT)
                {
                    y_angle = (y_angle - 5) % 360;
                }
                z_camera += 0.05; 
		...
		break;
            case GLUT_KEY_DOWN :
                ...
                z_camera -= 0.05; 
                cout << "GLUT_KEY_DOWN is down" << endl;
                glutPostRedisplay();
		break;                
        }
    }
    else
    {
        switch (key)
        {
            ...
            case GLUT_KEY_RIGHT :
                y_angle = (y_angle + 5) % 360;
                isDirectionalKeyDown = true;
                directional_key = GLUT_KEY_RIGHT;
		glutPostRedisplay();
		break;
            case GLUT_KEY_LEFT :
                y_angle = (y_angle - 5) % 360;
                isDirectionalKeyDown = true;
                directional_key = GLUT_KEY_LEFT;
		glutPostRedisplay();
		break;
            case GLUT_KEY_UP :
                z_camera += 0.05; 
                isDirectionalKeyDown = true;
                directional_key = GLUT_KEY_UP;
                glutPostRedisplay();
		break;
            case GLUT_KEY_DOWN :
                z_camera -= 0.05;
                isDirectionalKeyDown = true;
                directional_key = GLUT_KEY_DOWN;
                cout << "GLUT_KEY_DOWN is now down!" << endl;
                glutPostRedisplay();
		break;
        }
    }
